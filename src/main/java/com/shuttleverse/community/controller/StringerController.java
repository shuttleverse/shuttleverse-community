...
@RestController
@RequestMapping("/api/v1/stringers")
...
public class StringerController {
  ...
  @GetMapping("/{id}")
  public ResponseEntity<ApiResponse<Stringer>> getStringer(@PathVariable Long id) {
  ...
  @DeleteMapping("/{id}")
  @PreAuthorize("@stringerService.isOwner(#id, authentication.principal)")
  public ResponseEntity<ApiResponse<Void>> deleteStringer(@PathVariable Long id) {
